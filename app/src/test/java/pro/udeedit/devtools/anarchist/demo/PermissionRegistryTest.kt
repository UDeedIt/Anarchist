...
class PermissionRegistryTest {
    ...
    fun testStandardRegistryCompleteness() {
        ...
        assertTrue("Registry should contain Notification permission",
            standard.any { it.id == FeatureIds.ID_NOTIFICATIONS })

        assertTrue("Registry should contain Camera permission",
            standard.any { it.id == FeatureIds.ID_CAMERA })
    }

    @Test
    fun testRegistryIdsAreUnique() {
        val allIds = (PermissionRegistry.getStandardPermissions() +
                PermissionRegistry.getSpecialPermissions() +
                PermissionRegistry.getGroupedPermissions()).map { it.id }

        // Ensure no two features share the same ID
        assertEquals("Every feature in the registry must have a unique ID",
