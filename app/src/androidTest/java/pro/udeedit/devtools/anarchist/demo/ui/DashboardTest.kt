...
//    val composeTestRule = createComposeRule()
     ...
     */
    @Test
    fun testNavigationTabSwitching() = run {
...
//        }
     ...
     */
    @Test
    fun testInformationDialogVisibility() = run {
        ...
        step("Open Technical Information Dialog") {
            ...
            composeTestRule.onNodeWithTag("btn_info_$targetId", useUnmergedTree = true)
                .performClick()
        }
        ...
        step("Verify Dialog Content is visible") {
            flakySafely {
                ...
                composeTestRule.onNodeWithText("Technical Metadata", useUnmergedTree = true)
                    .assertIsDisplayed()
            }
        }
     ...
     */
    @Test
    fun testInformationDialogOnFirstCard() = run {
        step("Open Info Dialog for the first card in the sequence") {
             ...
             */
            val prefixMatcher = SemanticsMatcher("TestTag starts with 'btn_info_'") { node ->
                val tag = node.config.getOrElse(androidx.compose.ui.semantics.SemanticsProperties.TestTag) { "" }
                tag.startsWith("btn_info_")
            }

            composeTestRule.onAllNodes(prefixMatcher, useUnmergedTree = true)
                .onFirst()
                .performClick()
        }
